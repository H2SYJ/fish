...
public class StoreWorldEvent_卡牌商店 extends StoreWorldEvent {
    ...
    public void join(Player p1, Player p2) {
        this.renderer = new Renderer("欢迎光临");
    ...
    private void pay(Player player, List<Card> cards) {
        List<Choose> chooses = new ArrayList<>();
        AtomicBoolean continues = new AtomicBoolean(false);
        do {
            renderer.print("钱包：%sg", player.getData().getGold()).end();
            for (int i = 0; i < cards.size(); i++) {
                Card card = cards.get(i);
                int cost = price(card);
                Line line = renderer.print("%s）", i + 1);
                if (cost > player.getData().getGold()) {
                    ...
                    chooses.add(new Choose(String.valueOf(i + 1), input -> {
                        player.getData().modifyGold(-cost); // 付钱！
                        player.getDeck().add(card); // 交货！
                        cards.remove(card);
                    }));
                }
            ...
            do {
                controller = new Controller(player, "请选择");
